...
- **Фон по репозиторию.** Набор закрепляется за `владелец/имя` из git-remote, поэтому один и
  тот же проект, склонированный в разные папки, открывается одинаково. Требует
  компаньон-расширение: адрес репозитория есть только в git API, из вёрстки его не прочитать.
- **Точные данные вместо догадок.** С установленным компаньоном ветка, счётчик ошибок, язык
  активного файла и папка проекта приходят из настоящих API редактора, а не из его вёрстки —
  и не ломаются при обновлениях. Без компаньона всё работает по-прежнему; источник видно в
  «Система → Диагностика» строкой «Данные редактора».
