...
- **Наборы фонов** — 7 наборов (картинки для редактора / сайдбара / панели); выбор набора
  ...
  интерфейс под палитру набора. Имя набора можно **переименовать прямо в панели** (поле
  «Имя» в секции «Набор») — оно показывается на кнопке `BG` и в списках.
- **Пресеты** — сохранить весь текущий вид (набор, яркость, эффекты, терминал, акцент) под
  именем и переключаться между сохранёнными образами одним кликом. Хранятся отдельно от
  экспорта/импорта, до 24 штук.
- **Приглушение при печати** — пока набираешь текст, фоновая картинка редактора плавно
  тускнеет для читаемости и возвращается через паузу после последней клавиши (эффект
  «Тускнеть при печати»).
...
- **Авто-набор по времени суток** — днём один набор, ночью другой; наборы для дня и ночи
  выбираются в панели, **границы дня настраиваются** (ползунки «День с / до», часы 0–23,
  поддерживается интервал через полночь). Имеет приоритет над слайдшоу.
- **Светлая и тёмная тема** — поверхности «стекла», титлбара и скрима берут реальные цвета
  активной темы VS Code (переменные `--vscode-*`), а не зашитые константы; переключение темы
  применяется на лету. Сама панель «Фон и дизайн» тоже **подстраивается под тему** (на светлой
  теме — светлая палитра панели, а не тёмная поверх светлого редактора).
- **Мастер-выключатель** — одним тумблером (или `Ctrl+Alt+0`) снять весь фон и эффекты и получить
  обычный VS Code, не теряя настройки; включить обратно — тем же тумблером.
...
- **Картинка** — акцентный цвет интерфейса (курсор, скроллбар, вкладки, рамки…, свой на набор),
  кнопка **«Акцент из картинки»** (берёт доминирующий цвет фоновой картинки набора), и фильтры
  фоновой картинки **по зонам**: выбираешь зону — под неё настраиваются вписывание (cover /
  contain), яркость / насыщенность / размытие и **свой путь картинки** (`file:///…`, заменяет
  стандартную картинку зоны). Битая картинка (404) заменяется акцентной подложкой.
...
### Меню перекрашивается под акцент

Сама панель — часть оформления: подсказки «?», ползунки, чипы и кнопки берут цвет активного
набора. Ниже одно и то же меню (верх — наборы и яркость, низ — эффекты и терминал) в двух палитрах:

<div align="center">
<table>
  <tr>
    <th>Акцент «Маки» · <code>#f38ba8</code></th>
    <th>Акцент «Пурпур» · <code>#cba6f7</code></th>
  </tr>
  <tr>
    <td><img src="docs/screenshots/menu-poppy-top.png" alt="Панель в акценте «Маки»: наборы, слайдшоу, авто-по-времени, яркость, картинка" width="330"></td>
    <td><img src="docs/screenshots/menu-purple-top.png" alt="Панель в акценте «Пурпур»: наборы, слайдшоу, авто-по-времени, яркость, картинка" width="330"></td>
  </tr>
  <tr>
    <td><img src="docs/screenshots/menu-poppy-fx.png" alt="Панель в акценте «Маки»: сила эффектов, эффекты, терминал" width="330"></td>
    <td><img src="docs/screenshots/menu-purple-fx.png" alt="Панель в акценте «Пурпур»: сила эффектов, эффекты, терминал" width="330"></td>
  </tr>
</table>
<sub><b>Это лишь 2 из 7 палитр.</b> У каждого из 7 наборов свой акцент — панель перекрашивается вместе с интерфейсом при переключении набора.</sub>
</div>

Панель можно перетаскивать за заголовок; позиция запоминается. Закрыть — `Esc` или клик мимо.
Открыть панель с клавиатуры — `Ctrl+Alt+B`. Внутри панели работает `Tab` (фокус не выходит
за пределы окна), закрытие возвращает фокус на кнопку `BG`.
...
vscode-bg/
  ...
  package.json         ← npm-скрипты (build / test / check); зависимостей нет
  README.md            ← этот файл
  LICENSE              ← MIT
...
> `custom-bg.js` пересобран из `src/` (иначе PR не пройдёт CI).

Смоук-тест собирает модули, выполняет их в мини-DOM-стабе и проверяет генерацию CSS
(акцент набора, вписывание, url, прозрачность, авто-дим, 404-фолбэк, светлая/тёмная тема,
цвета поверхностей из `--vscode-*`, авто-набор по времени, листание наборов горячей клавишей).
Это ловит кросс-модульные `ReferenceError`, которые `node --check` не видит. Прогоняй его
после каждой правки в `src/**`.
