...
![sets](https://img.shields.io/badge/наборов-21-orange)
...
| Наборы без картинок | нет | нет | **да, 6 градиентных + 3 процедурных набора (ноль ассетов)** |
...
- **Наборы фонов** — 12 фото-наборов (картинки для редактора / сайдбара / панели), 6
  **генеративных** наборов-градиентов и 3 **процедурных** (звёздное поле, дюны, грейн —
  текстура рисуется на canvas, без единого ассета). Генеративные и процедурные грузятся
...
> **Процедурные наборы** (18–20) тоже без ассетов, но вместо плоского градиента рисуют
> текстуру на canvas (звёздное поле / волны-дюны / плёночный грейн) и кэшируют её; если canvas
> недоступен, зона мягко откатывается на градиент. В любую зону любого набора можно подложить
...
ним настройки разложены по **четырём вкладкам** (видна одна за раз, поэтому панель короткая), а
...
- **Набор** — плитки-чипы наборов с превью (наведение плавно примеряет набор, клик выбирает,
  ...
  не бывает), а также **Слайдшоу**, **По времени суток** и **По проекту** (фон под открытый проект
  + полоска-индикатор git-ветки).
- **Вид** — **Яркость набора** (прозрачность по зонам + авто-яркость), **Картинка** (акцентный
  цвет, фильтры и свой путь картинки по зонам) и **Эффекты** — 30+ тумблеров с поиском-фильтром
  сверху, ниже — «Сила» (числовые параметры) и стиль частиц. Параметры, зависящие от выключенного
  эффекта (число/стиль частиц, длительность помидора), показываются только когда эффект включён.
- **Терминал** — шрифт, лигатуры, свечение, курсор и выделение.
- **Система** — **Папка плагина** (путь к картинкам + «Разрешить сетевые картинки», по умолчанию
  выкл), **Пресеты** и **Поделиться** (сохранённые образы и обмен кодом), **Экспорт / импорт** в
  JSON с авто-резервом и кнопкой «Восстановить прежние настройки», сброс к дефолту.
...
<table>
  <tr>
    <th align="center">Набор</th>
    <th align="center">Вид</th>
    <th align="center">Терминал</th>
    <th align="center">Система</th>
  </tr>
  <tr valign="top">
    <td align="center"><img src="docs/screenshots/menu-sets-mauve.png" width="200" alt="Вкладка «Набор»: плитки наборов, слайдшоу, по времени суток, по проекту (сиреневый акцент)"></td>
    <td align="center"><img src="docs/screenshots/menu-view-mauve.png" width="200" alt="Вкладка «Вид»: яркость набора, картинка и 30+ эффектов с поиском (сиреневый акцент)"></td>
    <td align="center"><img src="docs/screenshots/menu-terminal-mauve.png" width="200" alt="Вкладка «Терминал»: шрифт, лигатуры, свечение, курсор (сиреневый акцент)"></td>
    <td align="center"><img src="docs/screenshots/menu-system-mauve.png" width="200" alt="Вкладка «Система»: папка плагина, пресеты, поделиться, экспорт/импорт (сиреневый акцент)"></td>
  </tr>
  <tr valign="top">
    <td align="center"><img src="docs/screenshots/menu-sets-maroon.png" width="200" alt="Вкладка «Набор» с розовым акцентом"></td>
    <td align="center"><img src="docs/screenshots/menu-view-maroon.png" width="200" alt="Вкладка «Вид» с розовым акцентом"></td>
    <td align="center"><img src="docs/screenshots/menu-terminal-maroon.png" width="200" alt="Вкладка «Терминал» с розовым акцентом"></td>
    <td align="center"><img src="docs/screenshots/menu-system-maroon.png" width="200" alt="Вкладка «Система» с розовым акцентом"></td>
  </tr>
</table>

<sub>Панель «Фон и дизайн» по вкладкам. Две строки — один и тот же вид в разных наборах: сверху
«Кот и звёзды» (сиреневый акцент), снизу «Меч в маках» (розовый). Выбор набора перекрашивает и
саму панель.</sub>
...
`[MoonLight custom-bg] v18 …`. Если её нет — скрипт не подгрузился (проверь путь и слэши).

**Как узнать версию.**
`Help → Toggle Developer Tools` → Console → строка `[MoonLight custom-bg] v18 …`.
